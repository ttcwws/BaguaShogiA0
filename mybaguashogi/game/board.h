...
static const int MAX_BIG_MOVE_NUM = 500;
...
namespace Location
{
    ...
    extern std::vector<std::tuple<int, int, int>> validPolicy;
...
struct Board
{
    ...
    static bool checkOnBoard(int x, int y);
    ...
    Hash128 getPosHash() const {
        ...
        int repcount = repetition[repind].count(rephash);
        return raw_pos_hash ^ ZOBRIST_REPETITION_HASH[repcount] ^ ZOBRIST_MOVENUM_HASH[movenum];
    }
    Hash128 getPosHash(Hash128 raw_hash) const {
        ...
        int repcount = repetition[repind].count(rephash);
        return raw_hash ^ ZOBRIST_REPETITION_HASH[repcount] ^ ZOBRIST_MOVENUM_HASH[movenum];
    }
  ...
  6 3 7
    */
    ...
    bool genLegalMove(std::vector<Move>& mvs, Player pla, bool recursion = false)const;
    std::vector<Move> genLegalMove()const;
    void genLegalPolicy(std::vector<size_t>& ret)const;
    ...
    int winner;
    std::unordered_map<uint64_t, uint8_t> first[2];
    std::unordered_set<uint64_t> repetition[2];
    std::vector<int> checkRecord[4];
    static void printlocation(Loc loc, Board& board, string prompt);
private:
    struct QuickCheckUndo {
        bool isDrop;
        bool kingMoved;
        Loc from;
        Loc to;
    ...
    void applyMoveForAttackCheck(const Move& mv, Player pla, QuickCheckUndo& undo) const;
    void undoMoveForAttackCheck(Player pla, const QuickCheckUndo& undo) const;
    bool wouldKingBeAttackedAfterMove(const Move& mv, Player pla) const;
